...
/*   Updated: 2024/03/25 19:17:23 by anacaro5         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_each_image(t_game *gamecontent, char position, int x, int y)
{
	...
	{
		mlx_image_to_window (gamecontent->mlx_ptr, gamecontent->image_villain,
			x, y);
		gamecontent->villain_count++;
	}
}
...
void	my_keyhook(mlx_key_data_t keydata, void *param)
{
	...
	put_sign(gamecontent, gamecontent->mapstruct);
	if (meet_villain(gamecontent) == 0)
	{
		close_hook(gamecontent);
		mlx_terminate(gamecontent->mlx_ptr);
		exit (EXIT_SUCCESS);
...
int	game_on(t_game *gamecontent, t_maps *mapcontent)
{
	...
	put_tex_to_ima_bonus(gamecontent);
